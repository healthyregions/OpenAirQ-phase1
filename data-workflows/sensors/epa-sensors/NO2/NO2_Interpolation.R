...
grid <-spsample(EPA.Points.Ozone, type = 'regular', n = 10000)

# runs the idw for the variable of interest in EPA.Points
idw <- idw(EPA.Points$no2 ~ 1, EPA.Points, newdata = grid)
...
tm_shape(masked_idw) + tm_raster("prediction", style = "quantile", n = 100, legend.show = FALSE) +
  tm_shape(EPA.Points) + tm_bubbles(size = "no2", col = "no2", palette = "Blues", 
...
colnames(chica.final) <- c("area_numbe", "no3")
...
# SAVE FOR REPORT
tm_shape(Chicago) + 
  tm_fill("no3", style = "quantile", n = 5, 
