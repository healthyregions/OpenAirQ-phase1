...
EPA.Points.Ozone <- readOGR(".", "Ozone")
str(EPA.Points@data)
EPA.Points@data
plot(EPA.Points)
...
idw <- idw(EPA.Points$Ozone ~ 1, EPA.Points, newdata = grid)
...
tm_shape(masked_idw) + 
  ...
  tm_shape(EPA.Points) + 
  tm_bubbles(col = "Ozone", 
             palette = "Blues", 
             scale = .8, 
             style = "quantile", 
             title.col = "2017 Mean Ozone") 

tmap_mode("plot")
# plot masked raster with Community Area overlay
# SAVE FOR REPORT
tm_shape(masked_idw, bbox=Chicago) + 
  tm_raster("prediction", style = "quantile", pal = "Blues", n = 5, legend.show = TRUE, title = "Ozone prediction") + 
...
colnames(chica.final) <- c("area_numbe", "O3")
...
# SAVE FOR REPORT
tm_shape(Chicago) + 
  tm_fill("O3", style = "quantile", n = 5, pal = "Blues",legend.show = TRUE, title = "Ozone by CA") + 
  tm_shape(Chicago) + tm_borders(alpha=.5,) + 
  tm_layout(frame = F, legend.position = c("left","bottom"))

# Writing the data
#write.csv(final, "PM5_aggregated.csv")
